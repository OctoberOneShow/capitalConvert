/*
 * Pet companion cases (part 1).
 * Cases 1-18 plus the page-effects guard, driving the real app sources.
 */
"use strict";

const { createEnvironment, appSource, stylesSource, PET_KEY, check, near, boot, adopt, statValue, fxCount, seedToolDom, run, notes } = require("./lib");
...
/* 1b. the page's click decoration stays out of the injected widget -------
 * initPet() runs before initAnimations(), so the page-wide ripple used to be
 * bound to every pet control as well. The ripple is positioned against the
 * nearest positioned ancestor and the pet buttons define none, so inside
 * #petButton it resolved to .pet-stage and inside .pet-action to the fixed
 * .pet-widget: the 108px circle scaled to 432px outside its button, overflowed
 * the scrolling .pet-panel and put a horizontal scrollbar across the bottom
 * edge of the card for the length of the animation. */
run("page-effects", () => {
  const env = createEnvironment({});
  /* a page-owned button exists before initAnimations() runs, so the page is
   * still guaranteed to have something for the ripple to decorate */
  const pageButton = env.document.createElement("button");
  pageButton.id = "pageRippleButton";
  env.document.body.appendChild(pageButton);
  env.load(appSource);
  env.domReady();
  adopt(env, "bracko", "Pip");

  env.dispatch(pageButton, "click");
  check("page-effects", "the page ripple still decorates page buttons",
    pageButton.getElementsByClassName("ripple").length === 1,
    String(pageButton.getElementsByClassName("ripple").length));

  const controls = env.byId("petWidget").querySelectorAll("button");
  check("page-effects", "the widget exposes its real controls",
    controls.length >= 20, String(controls.length));

  let ripples = 0;
  const offenders = [];
  controls.forEach((control) => {
    env.dispatch(control, "click");
    const found = control.getElementsByClassName("ripple").length;
    ripples += found;
    if (found) offenders.push(control.id || control.className);
  });
  check("page-effects", "no pet control inherits the page ripple",
    ripples === 0, `${ripples} ripple(s) in ${offenders.join(", ")}`);
  check("page-effects", "and nothing ripple-shaped is left inside the widget",
    env.byId("petWidget").querySelectorAll(".ripple").length === 0,
    String(env.byId("petWidget").querySelectorAll(".ripple").length));
  check("page-effects", "the ripple binding skips the widget",
    /closest\("#petWidget"\)/.test(appSource));

  /* .pet-panel is the card's scroll container. Its horizontal axis must never
   * be scrollable, so no wide child can put a bar across the bottom edge. */
  const panelRule = /\.pet-panel \{[\s\S]*?\}/.exec(stylesSource);
  check("page-effects", "the card can only scroll vertically",
    !!panelRule &&
      /overflow-x:\s*hidden/.test(panelRule[0]) &&
      /overflow-y:\s*auto/.test(panelRule[0]),
    panelRule ? panelRule[0].replace(/\s+/g, " ") : "no .pet-panel rule");

  /* the pet's own click feedback survived (the fix removed the page ripple,
   * not the pet's reaction) */
  const bubble = boot({});
  adopt(bubble, "bracko", "Pip");
  bubble.dispatch(bubble.byId("petButton"), "click");
  check("page-effects", "the pet still reacts to a click without the page ripple",
    bubble.byId("petButton").classList.contains("is-happy") &&
      bubble.byId("petStatus").textContent.length > 0,
    bubble.byId("petStatus").textContent);
  bubble.timers.advance(6000);
  check("page-effects", "the pet still runs clean afterwards",
    bubble.timers.pendingIntervals() === 1,
    String(bubble.timers.pendingIntervals()));
});
